#!/usr/bin/env python3
"""
Fixed Comprehensive Integration Test for Intelligence Module
...
async def test_base_components():
    ...
    try:
        ...
        print(f"   Score: {complexity.score:.2f}")  # Changed from overall_score
        print(f"   Methodology: {complexity.methodology.value}")
        print(f"   Estimated Time: {complexity.estimated_duration_minutes} minutes")  # Changed attribute
...
async def test_vector_enhanced_components():
    ...
    try:
        # Test imports
        from vector_enhanced_domain_expert import VectorEnhancedDomainExpert, VectorEnhancedBusinessIntent
        from vector_enhanced_complexity_analyzer import VectorEnhancedComplexityAnalyzer, VectorEnhancedComplexityScore
        ...
        if vector_expert.embedder:
            ...
            # Access through base_intent
            print(f"   Query: {query}")
            print(f"   Domain: {enhanced_intent.base_intent.primary_domain.value}")
...
async def test_integrated_workflow():
    ...
    try:
        # Import with sys.path manipulation for standalone execution
        import sys
        ...
        print(f"   - Score: {result.complexity_score.score:.2f}")
        print(f"   - Estimated Time: {result.complexity_score.estimated_duration_minutes} minutes")
...
async def test_vector_integration():
    ...
    try:
        ...
        if vector_expert.embedder:
            enhanced_intent = await vector_expert.classify_business_intent_with_vectors(query)
            print(f"   ✅ Enhanced classification: {enhanced_intent.base_intent.primary_domain.value}")
            print(f"   Confidence boost: {enhanced_intent.confidence_boost:.2%}")
            
            # Use base_intent for compatibility
        ...
        if vector_analyzer.embedder:
            ...
            # Create base complexity score for compatibility
            from complexity_analyzer import ComplexityScore
            base_complexity = ComplexityScore(
                level=enhanced_complexity.level,
                methodology=enhanced_complexity.methodology,
                score=enhanced_complexity.overall_score,
                dimension_scores=enhanced_complexity.dimension_scores,
                estimated_duration_minutes=enhanced_complexity.estimated_duration_minutes,
                estimated_queries=enhanced_complexity.estimated_queries,
                estimated_services=enhanced_complexity.estimated_services,
                confidence=enhanced_complexity.confidence,
                risk_factors=enhanced_complexity.risk_factors
            )
        else:
...
async def test_component_composition():
    """Test that all components can be composed together properly."""
    print(f"\n{BLUE}=== Testing Component Composition ==={RESET}")
    ...
    try:
        ...
        print(f"{GREEN}✅ All components imported successfully{RESET}")
        
        # Test that components can be instantiated together
        components = {
            "domain_expert": DomainExpert(),
            "complexity_analyzer": ComplexityAnalyzer(),
            "business_context": BusinessContextAnalyzer(),
            "hypothesis_generator": HypothesisGenerator(),
            "pattern_recognizer": PatternRecognizer(),
            "vector_domain_expert": VectorEnhancedDomainExpert(),
            "vector_complexity": VectorEnhancedComplexityAnalyzer(),
            "lancedb_patterns": LanceDBPatternRecognizer()
        }
        
        print(f"\n{YELLOW}Component instances created:{RESET}")
        for name, component in components.items():
            print(f"   ✅ {name}: {component.__class__.__name__}")
        
        # Test a simple query through multiple components
        query = "What factors are affecting our quarterly revenue?"
        
        # Domain classification
        intent = components["domain_expert"].classify_business_intent(query)
        print(f"\n{YELLOW}Pipeline test:{RESET}")
        print(f"   1. Intent: {intent.primary_domain.value} ({intent.analysis_type.value})")
        
        # Complexity analysis
        complexity = components["complexity_analyzer"].analyze_complexity(intent, query)
        print(f"   2. Complexity: {complexity.level.value} (score: {complexity.score:.2f})")
        
        # Context adaptation
        strategy = components["business_context"].analyze_context(
            business_intent=intent,
            complexity_level=complexity.level,
            base_methodology=complexity.methodology,
            user_id="test",
        ...
        print(f"   3. Strategy: {strategy.adapted_methodology.value}")
        
        # Hypothesis generation
        hypotheses = components["hypothesis_generator"].generate_hypotheses(
            business_intent=intent,
            contextual_strategy=strategy,
            complexity_level=complexity.level
        )
        print(f"   4. Hypotheses: {len(hypotheses.secondary_hypotheses) + 1} generated")
        
        # Pattern recognition
        patterns = components["pattern_recognizer"].recognize_patterns(
            business_intent=intent,
            contextual_strategy=strategy
        )
        print(f"   5. Patterns: {len(patterns.domain_patterns)} domain, {len(patterns.query_patterns)} query")
        
        print(f"\n{GREEN}✅ All components compose properly!{RESET}")
        return True
        
    except Exception as e:
        print(f"{RED}❌ Component composition test failed: {e}{RESET}")
        import traceback
        traceback.print_exc()
        return False
...
async def main():
    ...
    tests = [
        ...
        ("Component Composition", test_component_composition)
    ]
    ...
    print(f"✅ Base components: Domain Expert, Complexity Analyzer, Business Context")
    print(f"✅ Strategy components: Hypothesis Generator, Pattern Recognizer")
    print(f"✅ Vector enhancements: Enhanced Domain Expert, Enhanced Complexity")
    print(f"✅ Cross-module: LanceDB Pattern Recognizer")
    print(f"✅ Pipeline: All components can work together")
